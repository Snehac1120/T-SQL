...
SELECT S.RNO,BRANCH,NAME,SPI,RESULTID,R.RNO FROM STUDENT_INFO S
INNER JOIN RESULT R
ON S.RNO=R.RNO;

--3. Perform the left outer join on Student and Result tables. 
SELECT S.RNO,BRANCH,NAME,SPI,RESULTID FROM STUDENT_INFO S
...
SELECT S.RNO,BRANCH,NAME,SPI FROM STUDENT_INFO S
INNER JOIN RESULT R
ON S.RNO=R.RNO;
...
SELECT AVG(SPI) AS AVG_SPI,BRANCH FROM STUDENT_INFO S
INNER JOIN RESULT R
ON S.RNO=R.RNO
GROUP BY BRANCH;
...
SELECT MAX(SPI) AS MAX_SPI ,MIN(SPI) AS MIN_SPI,BRANCH FROM STUDENT_INFO S
INNER JOIN RESULT R
ON S.RNO=R.RNO
GROUP BY BRANCH;

--14. Display branch-wise student count in descending order.
SELECT COUNT(S.RNO) AS CNT,BRANCH FROM STUDENT_INFO S
INNER JOIN RESULT R
ON S.RNO=R.RNO
GROUP BY BRANCH
...
SELECT SUM(SPI) AS SUM_SPI,BRANCH FROM STUDENT_INFO S
INNER JOIN RESULT R
ON S.RNO=R.RNO
GROUP BY BRANCH;
...
SELECT MAX(SPI) AS MAX_SPI ,BRANCH FROM STUDENT_INFO S
INNER JOIN RESULT R
ON S.RNO=R.RNO
GROUP BY BRANCH
...
SELECT BRANCH,(MAX(SPI)-MIN(SPI)) AS DIFF FROM STUDENT_INFO S
INNER JOIN RESULT R
ON S.RNO=R.RNO
GROUP BY BRANCH
